...
- O indicador **“A receber” do Dashboard** e a abertura da tela Contas a Receber mostram a mesma coisa: a soma das contas com `status = aberto`, geradas por OS já entregues. OS ainda em produção são previsão operacional e não entram no saldo financeiro. Na Lista de OS, o card **“Total das OS filtradas”** é operacional: soma exatamente as OS resultantes de mês, busca e status, independentemente da baixa. O indicador **“Receita bruta no mês”** do Dashboard segue essa regra operacional para o mês corrente.
- Toda somatória operacional usa valor cheio para Clientes e comissão para Parceiros, ignora OS canceladas e nunca fica negativa quando o desconto supera o subtotal. Essa mesma regra é compartilhada pela Lista de OS, extrato por entidade, Dashboard e relatórios/PDFs.
...
**Criar um usuário novo exige um administrador da própria empresa.** Em `Configurações → Usuários`, ele informa nome, e-mail e papel; a Edge Function `invite-user` valida sua sessão, cria o acesso no Supabase Auth, vincula o perfil ao mesmo `empresa_id` e envia o kit de boas-vindas. A chave `service_role` fica exclusivamente nos secrets do Supabase. Não há cadastro público. O convidado precisa criar uma senha pessoal antes de acessar qualquer área interna (`user_metadata.must_change_password`). O primeiro administrador de uma empresa nova continua sendo provisionado pela Lotus, porque essa operação define o tenant inicial.

**O primeiro administrador conclui a configuração inicial antes de operar.** Depois de criar a senha, ele confirma nome do laboratório, nome do responsável, telefone e e-mail; documento e endereço são opcionais. Enquanto `empresas.onboarding_concluido = false`, as telas internas redirecionam para essa etapa. Cada usuário também precisa informar um CPF/CNPJ válido em “Meu Perfil” antes de acessar as demais telas. A própria pessoa pode alterar nome e documento fiscal; nível de acesso e vínculo com a empresa continuam protegidos. A mesma página exibe o plano, o status da assinatura e o histórico de faturas/notas fiscais da empresa.
