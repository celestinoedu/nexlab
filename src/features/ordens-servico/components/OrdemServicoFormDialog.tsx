...
export function OrdemServicoFormDialog({ open, onOpenChange, ordem }: OrdemServicoFormDialogProps) {
  ...
  function aplicarPrecoSugerido(index: number, servicoId: string) {
    ...
    const precoAplicado = precoEspecifico ?? servico.preco_padrao
    if (entidadeSelecionada?.tipo === 'parceiro') {
      setValue(`itens.${index}.valor_unitario`, servico.preco_padrao)
      setValue(`itens.${index}.valor_comissao`, precoAplicado)
    } else {
      setValue(`itens.${index}.valor_unitario`, precoAplicado)
      setValue(`itens.${index}.valor_comissao`, null)
    }
  }

  // Na criação, o usuário pode escolher os serviços antes da entidade. Assim
  // que a tabela do Cliente/Parceiro carregar, reaplica a mesma prioridade de
  // preço a todos os itens: tabela específica e, se não houver, catálogo.
  // Em edição preserva os valores históricos já gravados na OS.
  React.useEffect(() => {
    if (!open || isEditing || !entidadeId || !servicos || !precos || !itensAtuais?.length) return
    itensAtuais.forEach((item, index) => {
      if (item.servico_id) aplicarPrecoSugerido(index, item.servico_id)
    })
    // Os IDs bastam: a aplicação de preço não deve reagir à edição manual dos valores.
    // oxlint-disable-next-line react-hooks/exhaustive-deps
  }, [open, isEditing, entidadeId, entidadeSelecionada?.tipo, servicos, precos, JSON.stringify(itensAtuais?.map((i) => i.servico_id))])
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex h-[calc(100%-1rem)] max-h-[calc(100%-1rem)] w-[calc(100%-1rem)] max-w-none flex-col overflow-hidden p-0 sm:h-[calc(100%-3rem)] sm:max-h-[calc(100%-3rem)] sm:w-[calc(100%-3rem)]">
        <DialogHeader className="m-0 shrink-0 border-b border-slate-100 px-5 py-4 pr-12 sm:px-6">
          <DialogTitle>
            {ordem
            ...
            <div className="grid grid-cols-1 gap-4 border-t border-slate-100 pt-4 sm:grid-cols-2">
              ...
              <div className="flex flex-col justify-end rounded-xl bg-brand-50 px-3 py-2">
                <div className="flex items-center gap-1 text-xs font-medium text-brand-700">
                  ...
                  <TotalInfo titulo="Total da OS">Prévia dos itens preenchidos: quantidade × valor para Cliente ou quantidade × comissão para Parceiro, menos o desconto, com mínimo de zero. O valor vem primeiro da tabela do Cliente/Parceiro; sem preço específico, usa o preço padrão do catálogo. Este valor não indica saldo pendente; se a OS for cancelada, será excluída dos totais operacionais.</TotalInfo>
                </div>
    ...
    <div className="flex flex-col gap-2 rounded-xl border border-slate-200 p-3">
      ...
      {temPrecoEspecifico === false && (
        <div className="flex items-start gap-1.5 text-xs text-warning-700">
          ...
          <span>
            Sem preço específico na tabela deste {ehParceiro ? 'Parceiro' : 'Cliente'} — usando o preço padrão do catálogo.
          </span>
        </div>
      )}
    </div>
